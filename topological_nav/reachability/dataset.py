...
class DatasetSourceTargetPair(DatasetVisualGibson):
    ...
    def _draw_sample_diff_traj(self, idx):
        ...
        dst_dataset_idx, dst_traj_id, dst_idx = self._locate_sample_single_map(
            idx2, map_name
        )
    ...
    def _compute_overlap(self, map, pos1, heading1, pos2, heading2):
        return map.view_overlap(
            pos1, heading1, self.fov, pos2, heading2, self.fov, mode="plane"
        )

    def reachable(self, map, src_sample, dst_sample):
        overlap_ratios = self._compute_overlap(
            map,
            src_sample["pos"],
            src_sample["heading"],
            dst_sample["pos"],
        ...
        if overlap_ratios[0] < 0.1 or overlap_ratios[1] < 0.1:
            ...
            overlap_ratios[0] > self.reach_overlap_thres[0]
        ...
        while n < step_limit and not stop:
            ...
            if not stop:
                ...
                overlap_ratios = self._compute_overlap(
                    map, agent.pos, agent.heading, goal_pos, goal_heading
                )

                if overlap_ratios[0] < 0.05 and overlap_ratios[1] < 0.05:
                    if self.debug:
                        print(
                            "overlap too small %.2f %.2f"
        ...
        if self.debug:
            ...
            vis_img[
                ...
                w // 2 - box_size[0] // 2 : w // 2 + box_size[0] // 2,
            ] = (255, 255, 255)
    ...
    def _init_once(self, seed):
        if self.first:
            self.motion_policy = MotionPolicyClient(
                self.motion_policy_weights_file, self.mp_server_addr
            )
...
class DatasetSourceTargetPairV3(DatasetSourceTargetPair):
    ...
    def _jitter(self, sample, map):
        ...
        heading_perp = rotate_2d(
            np.array([math.cos(heading), math.sin(heading)], np.float32), math.pi / 2
        )
        ...
        for i in range(max_try):
            ...
            new_sample["pos"] = pos + heading_perp * np.clip(
                self.rng.randn() * 0.3, -1.0, 1.0
            )
    ...
    def _make_sample_seq(self, traj, idx):
        ...
        if self.future:
            for i in range(self.n_frame - 1):
                samples.append(
                    traj[min(idx + (i + 1) * self.frame_interval, len(traj) - 1)]
                )
        return samples
...
class DatasetSourceTargetPairMultiframeDstRecording(Dataset):
    def __init__(
        self, recording_dir, resolution, positive=True, negative=True, **kwargs
    ):
        super(DatasetSourceTargetPairMultiframeDstRecording, self).__init__(**kwargs)
        self.recording_dir = recording_dir
        self.resolution = resolution

        self.positive_samples = self._read_images(
            os.path.join(self.recording_dir, "positive")
        )
        self.negative_samples = self._read_images(
            os.path.join(self.recording_dir, "negative")
        )
        ...
        self.sample_weights = [1.0 / len(self.positive_samples)] * len(
            self.positive_samples
        ) + [1.0 / len(self.negative_samples)] * len(self.negative_samples)

        print(
            "%d positive samples %d negative samples"
    ...
    def _read_images(self, dir):
        ...
        for fn in img_files:
            ...
            if _1 == "ob":
                ...
                imgs = [
                    self._normalize(self._maybe_resize(_))
    ...
    def __getitem__(self, idx):
        if idx < len(self.positive_samples):
            return (
                self.positive_samples[idx][0],
                self.positive_samples[idx][1],
                np.array([1.0], np.float32),
            )
        else:
            i = idx - len(self.positive_samples)
            return (
                self.negative_samples[i][0],
                self.negative_samples[i][1],
...
class DatasetSourceTargetPairMultiframeDstNaive(DatasetSourceTargetPairMultiframeDst):
    ...
    def __init__(
        self, positive_interval, negative_interval, randomize_interval=False, **kwargs
    ):
        """
        ...
        while True:
            ...
            if self.randomize:
                interval = self.rng.randint(
                    self.negative_interval, max(len(traj), self.negative_interval + 1)
                )
            else:
...
class DatasetSourceTargetPairMultiframeDstLidar(DatasetSourceTargetPairMultiframeDst):
    ...
    def _get_ob(self, map_name):
        self.agent._measure()
        return self._apply_dropout(self._inject_noise(self.agent.depth_local)).astype(
            np.float32
        )

    def _compute_overlap(self, map, pos1, heading1, pos2, heading2):
        return map.view_overlap(
            pos1, heading1, self.fov, pos2, heading2, self.fov, mode="lidar"
        )
    ...
    def _make(self, map_name, samples, **kwargs):
        ...
        for sample in samples:
            ...
            agent._measure()
            depth = self._apply_dropout(self._inject_noise(agent.depth_local)).astype(
                np.float32
            )
...
class DatasetSourceTargetPairMultiframe(DatasetSourceTargetPair):
    ...
    def __getitem__(self, idx):
        ...
        if len(obs) > self.n_frame:
            ...
            len(obs) > 2
        ):  # Skipping the first frame results in at least one frame available.
            zero_imgs = np.zeros(
                (self.n_frame - len(obs) + 1,) + obs[0].shape, obs[0].dtype
            )
...
if __name__ == "__main__":
    ...
    def test_multiframe():
        dataset = DatasetSourceTargetPairMultiframe(
            ...
            motion_policy_weights_file=os.path.dirname(__file__)
            + "/../imitation/experiments/simple/imitation-reverseprob0.3-use_gtwp-model.10",
            maps=None,
            agent_name="classic_240fov_minirccar",
            net_config=EasyDict({"input_resolution": 64}),
            hd5_files=[
                "/home/xymeng/dev/merlin/data/rmp/gibson/minirccar_agent_local_240fov_house31_dest_replan/train_1.hd5"
            ],
        ...
        for (
            idx,
            (batch_src_imgs, batch_dst_imgs, batch_reachability, batch_obs, batch_mask),
        ) in enumerate(loader):
            ...
            cv2.imshow(
                ...
                np.concatenate(
                    [
                        np.concatenate(src_imgs, axis=1),
    ...
    def test_v3():
        dataset = DatasetSourceTargetPairV3(
            ...
            motion_policy_weights_file=os.path.dirname(__file__)
            + "/../imitation/experiments/house31/pair/gtwp-normwp-farwp-jitter-weightedloss-checkwp-model.5",
            # motion_policy_weights_file=os.path.dirname(
            ...
            net_config=EasyDict({"input_resolution": 64}),
            hd5_files=[
                "/home/xymeng/dev/merlin/data/rmp/gibson/minirccar_agent_local_240fov_house31_farwp/train_1.hd5"
            ],
            assets_dir=os.path.dirname(__file__) + "/../../../gibson/assets/dataset",
            camera_pos=(0.065, 0.00),
            camera_z=1.0,
        ...
        for idx, (batch_src_imgs, batch_dst_imgs, batch_reachability) in enumerate(
            loader
        ):
            src_imgs = batch_src_imgs[0].data.cpu().numpy().transpose((1, 2, 0))
            dst_imgs = batch_dst_imgs[0].data.cpu().numpy().transpose((1, 2, 0))
    ...
    def test_multiframe_dst_naive():
        dataset = DatasetSourceTargetPairMultiframeDstNaive(
            ...
            sample_diff_traj_prob=0.5,
            motion_policy_weights_file=os.path.dirname(__file__)
            + "/../imitation/experiments/12env_v2/multiframe_dst/future/pair/pred_proximity/pred_heading_diff/conv/gtwp-normwp-farwp-jitter-weightedloss-checkwp-nf6-interval3-dmax3-model.6",
            maps=None,
            agent_name="classic_240fov_minirccar",
            net_config=EasyDict({"input_resolution": 64}),
            hd5_files=glob.glob(
                "../../../../../../data/rmp/gibson/minirccar_agent_local_240fov_house24_farwp/train_*.hd5"
            ),
            assets_dir=os.path.dirname(__file__) + "/../../../gibson/assets/dataset",
            camera_pos=(0.065, 0.00),
            camera_z=1.0,
    ...
    def test_multiframe_dst():
        ...
        dataset = DatasetSourceTargetPairMultiframeDst(
            ...
            swap_src_tgt_prob=0.0,
            motion_policy_weights_file=os.path.dirname(__file__)
            + "/../imitation/experiments/12env_v2/multiframe_dst/future/pair/pred_proximity/pred_heading_diff/conv/gtwp-normwp-farwp-jitter-weightedloss-checkwp-nf6-interval3-dmax3-z0228-model.666666",
            maps=None,
            agent_name="classic_240fov_minirccar",
            net_config=EasyDict({"input_resolution": 64}),
            hd5_files=[
                "../../../../../../data/rmp/gibson/minirccar_agent_local_240fov_house24_farwp/train_1.hd5"
            ],
            assets_dir=os.path.dirname(__file__) + "/../../../gibson/assets/dataset",
            camera_pos=(0.065, 0.00),
            camera_z=0.228,
    ...
    def test_multiframe_dst_lidar():
        dataset = DatasetSourceTargetPairMultiframeDstLidar(
            ...
            swap_src_tgt_prob=0.0,
            motion_policy_weights_file=os.path.dirname(__file__)
            + "/../imitation/experiments/12env_v2_laserscan/multiframe_dst/future/pair/pred_proximity/pred_heading_diff/conv/gtwp-normwp-farwp-jitter-weightedloss-checkwp-nf6-interval3-dmax3-model.10",
            maps=None,
            agent_name="classic_240fov_minirccar_64",
            net_config=EasyDict({"input_resolution": 0}),
            hd5_files=[
                get_project_root()
                + "/data/rmp/gibson/minirccar_agent_local_240fov_house31_fixedwpdist/train_1.hd5"
            ],
            assets_dir=os.path.dirname(__file__) + "/../../../gibson/assets/dataset",
            n_filler_server=0,
            n_sim_per_map=0,
        ...
        for idx, (batch_src_scans, batch_dst_scans, batch_reachability) in enumerate(
            loader
        ):
            r = batch_reachability[0].item()
            print("reachability", r)
    ...
    def test_multiframe_dst_recording():
        dataset = DatasetSourceTargetPairMultiframeDstRecording(
            "analysis/manual_labels", 64, negative=False
        )
        loader = DataLoader(dataset, batch_size=1, shuffle=True)
