        ...
        if u[0] == v[0]:
            return {
                ...
                "linewidth": line_widths[
                    (u[0] // (cm.N * len(arrow_styles))) % len(line_widths)
                ],
    ...
    def visualize_path(
        ...
        if start_pos is not None:
            ...
            if start_heading is not None:
                vis.plotter.fixed_arrow(
                    "start",
                    start_pos[0],
                    start_pos[1],
                    math.cos(start_heading),
                    math.sin(start_heading),
                    color=(0, 0.5, 0),
                )
        if goal_pos is not None:
            ...
            if goal_heading is not None:
                vis.plotter.fixed_arrow(
                    ...
                    goal_pos[0],
                    goal_pos[1],
                    math.cos(goal_heading),
                    math.sin(goal_heading),
                    color=(0.5, 0, 0),
                )
        for i in range(len(path) - 1):
            self._draw_edge(vis, path[i], path[i + 1], traj_info_dict)
...
class NavGraph(NavGraphBase):
    ...
    def save(self, out_file):
        with open(out_file, "wb") as f:
            pickle.dump(
                {"trajs": self.trajs, "graph": self.graph, "extra": self.extra}, f
            )
    ...
    def load(self, save_file):
        with open(save_file, "rb") as f:
            ...
            self.reuse_terminal_nodes = self.extra.get(
                "reuse_terminal_nodes", self.reuse_terminal_nodes
            )
            self.reuse_start_nodes = self.extra.get(
                "reuse_start_nodes", self.reuse_start_nodes
            )
            self.reuse_goal_nodes = self.extra.get(
                "reuse_goal_nodes", self.reuse_goal_nodes
            )
            self.reuse_edge_prob_mode = self.extra.get(
                "reuse_edge_prob_mode", self.reuse_edge_prob_mode
            )
    ...
    def _find_reusable_nodes(self, graph, traj, anchor_obs, anchor_idxs):
        ...
        for i in it:
            ...
            if i == len(anchor_obs) - 1:
                ...
                ob_next_dst_repr = self.sparsifier.get_dst_repr(
                    traj, anchor_idxs[i + 1]
                )
            ...
            for j in range(len(vs)):
                ...
                if (
                    r1 > self.reuse_thres
                    and r2 > self.reuse_thres
                    and r1 * r2 > best_r1 * best_r2
                ):
    ...
    def _find_new_edges_sptm(self, nodes, new_g, old_g, sample_limit=100):
        ...
        def make_seq(tid, apos):
            return [
                self.trajs[tid][np.clip(_, 0, len(self.trajs[tid]) - 1)]
        ...
        for traj_id, anchor_idx in all_nodes[:sample_limit]:
            ...
            for traj_id2, anchor_idx2 in nodes:
                anchor_pos2 = self.trajs[traj_id2].index(
                    (traj_id2, anchor_idx2)
                )  # TODO: slow?
        ...
        for i in range(len(rs)):
            if existing_nodes[i][0] != goal_node:
                if rs[i] > edge_add_thres:
                    ...
                    g.add_edge(
                        start_node, existing_nodes[i][0], weight=-math.log(rs[i])
                    )
        best_ob_entry = np.argmax(rs)
        print(
            "best ob entry node: %s %.3f"
            % (existing_nodes[best_ob_entry][0], rs[best_ob_entry])
        )
    ...
    def add_traj(self, traj, find_new_edge_samples=100):
        ...
        if self.reuse_edge_prob_mode == "hard":
            ...
            obs = [
                g.nodes[nodes[anchor_idxs[_]]]["ob_repr"]
                for _ in range(len(anchor_idxs) - 1)
            ]
            dsts = [
                g.nodes[nodes[anchor_idxs[_]]]["dst_repr"]
                for _ in range(1, len(anchor_idxs))
            ]
            probs = self.compute_reachability_batch(obs, dsts)
        else:
            raise RuntimeError(
                "Unsupported reuse_edge_prob_mode %s" % self.reuse_edge_prob_mode
            )
        ...
        if self.delta_tw is not None:
            new_edges = self._find_new_edges_sptm(
                new_nodes, g, old_g, sample_limit=find_new_edge_samples
            )
        else:
            new_edges = self._find_new_edges(
                new_nodes, g, old_g, sample_limit=find_new_edge_samples
            )
...
class NavGraphSPTM(NavGraphBase):
    ...
    def load(self, save_file):
        with open(save_file, "rb") as f:
            ...
            self.delta_tl, self.delta_tw, self.s_shortcut = [
                self.extra[_] for _ in ["delta_tl", "delta_tw", "s_shortcut"]
            ]
    ...
    def _localize_dst(self, dst_repr, topk=1):
        ...
        for node in nodes:
            rs.append(
                self.compute_reachability(self.graph.nodes[node]["ob_repr"], dst_repr)
            )
        closest_idxs = np.argsort(rs)[-topk:]
    ...
    def _create_shortcuts(self, old_g, new_g, nodes, sample_limit=100):
        def compute_median_sequence_reachability(seq1, seq2):
            ...
            dsts = [
                self.retrieval_net.get_dst_repr(self.full_traj_obs[traj_id], ob_idx)
        ...
        for traj_id, anchor_idx in all_nodes[:sample_limit]:
            seq1 = [
                (traj_id, np.clip(_, 0, len(self.full_traj_obs[traj_id]) - 1))
                for _ in range(
                    anchor_idx - self.delta_tw, anchor_idx + self.delta_tw + 1
                )
            ]
            for traj_id2, anchor_idx2 in nodes:
                seq2 = [
                    (traj_id2, np.clip(_, 0, len(self.full_traj_obs[traj_id2]) - 1))
                    for _ in range(
                        anchor_idx2 - self.delta_tw, anchor_idx2 + self.delta_tw + 1
                    )
                ]
...
def update_nav_graph(
    ...
    def _find_good_start(traj):
        ...
        for i in range(1, len(traj)):
            ...
            dp = (math.cos(heading) * dx + math.sin(heading) * dy) / math.sqrt(
                dx ** 2 + dy ** 2
            )
            if dp > 0.01:
                break
    ...
    for idx, tid in enumerate(traj_ids):
        ...
        if tid in id_mapping:
            ...
            graph_traj_id = graph.add_traj(
                obs, find_new_edge_samples=find_new_edge_samples
            )
            id_mapping[tid] = graph_traj_id

        if isinstance(traj, np.ndarray):
            # Real trace
            traj_info_dict[id_mapping[tid]] = {"samples": traj}
        else:
            # Simulation trace
