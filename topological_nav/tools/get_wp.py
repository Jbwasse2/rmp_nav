...
def run_agent_online_planning(
    ...
    def post_step(step, path, extra, screenshot_repeat=1):
        ...
        vis.plotter.scatter(
            "active_wp", active_wp[0], active_wp[1], marker="o", color="r", s=100.0
        )
        ...
        cv2.putText(
            vis_img,
            "state: %s" % state_strs[follower.state],
            (5, text_y),
            cv2.FONT_HERSHEY_SIMPLEX,
            1.0,
            (0, 0, 255),
        ...
        if extra["reachability"] is not None:
            cv2.putText(
                vis_img,
                "reachability: %.2f" % extra["reachability"],
                (5, text_y),
                cv2.FONT_HERSHEY_SIMPLEX,
                1.0,
                (0, 0, 255),
        ...
        vis_anchor = cv2.cvtColor(
            cv2.resize(
                active_anchor["ob_repr"].transpose((1, 2, 0)),
                dsize=(0, 0),
                fx=4.0,
                fy=4.0,
            ),
    ...
    def optimize_plan(ob, prev_prob):
        path, log_prob, _ = nav_graph.find_path(
            ob, goal_repr, edge_add_thres=0.5, allow_subgraph=True
        )
        if path is None:
            return None
        ...
        if path_vis_dir is not None:
            ...
            cv2.imwrite(
                ...
                (cv2.cvtColor(ob.transpose(1, 2, 0), cv2.COLOR_RGB2BGR) * 255).astype(
                    np.uint8
                ),
            )
            ...
            cv2.imwrite(
                ...
                (
                    cv2.cvtColor(next_anchor.transpose(1, 2, 0), cv2.COLOR_RGB2BGR)
                    * 255
                ).astype(np.uint8),
            )
            ...
            if step == 0:
                cv2.imwrite(
                    ...
                    (
                        cv2.cvtColor(goal_ob.transpose(1, 2, 0), cv2.COLOR_RGB2BGR)
                        * 255
                    ).astype(np.uint8),
                )
    ...
    while True:
        ...
        if (
            follower.state == follower.DEAD_RECON
        ...
        if agent.reached_goal(relax=3.0):
            ...
            overlap_ratios = compute_overlap(
                agent.pos, agent.heading, goal_pos, goal_heading
            )
...
def exec_plan2(
    ...
    **kwargs
    ...
    result = {"log_likelihood": log_likelihood, "success": False}
            ...
            for i in range(len(path) - 1):
                ...
                print(
                    ...
                    % (
                        ...
                        path[i + 1],
                        math.exp(
                            -nav_graph.graph.edges[path[i], path[i + 1]]["weight"]
                        ),
                        p,
                    )
                )
...
def exec_plan_to_destination(
    model, nav_graph, start_pos, start_heading, goal, map, dataset, agent, **kwargs
):
    ...
    return exec_plan2(
        ...
        start_pos,
        start_heading,
        [_["pos"] for _ in goal_samples],
        [_["heading"] for _ in goal_samples],
        map,
        nav_graph.extra["traj_info"],
        **kwargs
    )
...
if __name__ == "__main__":
    ...
    gflags.DEFINE_float(
        "start_heading", None, "In degrees. Used for plan_to_dest_single task."
    )
    gflags.DEFINE_float(
        "graph_subset_ratio", 1.0, "Use this subset of trajectories of the graph."
    )
    gflags.DEFINE_boolean(
        "jitter_start", False, "Jitter the starting position and heading."
    )
    gflags.DEFINE_boolean(
        "shortcut_bad_path", False, "True to treat a badly planned path as failure."
    )
    ...
    def frontend():
        ...
        if "naive" in FLAGS.model:
            nav_graph = NavGraphSPTM.from_save_file(
                model["sparsifier"], model["motion_policy"], FLAGS.graph_save_file
            )
        else:
        ...
        # Doesn't work, need to train a new model :D
        #        model["agent"] = "terrasentia"
