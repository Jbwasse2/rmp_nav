...
from topological_nav.reachability.planning import (
    NavGraph,
    NavGraphSPTM,
    update_nav_graph,
)
...
class DatasetRealTrace(object):
    ...
    def locate_traj(self, traj_id):
        ...
        dtype = np.dtype(
            [("pos", (np.float32, 2)), ("heading", np.float32), ("filenames", "U128")]
        )
...
if __name__ == "__main__":
    ...
    if graph_type == "ours":
        nav_graph = NavGraph(
            model["sparsifier"], model["motion_policy"], **graph_config["graph_kwargs"]
        )
    elif graph_type == "SPTM":
        nav_graph = NavGraphSPTM(
            model["sparsifier"], model["motion_policy"], **graph_config["graph_kwargs"]
        )
    else:
...
##    data_source = graph_config.get("data_source", "real")
    ...
    print(
        "number of traj images: %d"
        % sum([len(_["samples"]) for _ in traj_info_dict.values()])
    )
    ...
    nav_graph.visualize(
        map, traj_info_dict, save_file=os.path.dirname(FLAGS.save_file) + "/graph.svg"
    )
