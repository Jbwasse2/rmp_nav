...
common_kwargs = {
    ...
    "n_filler_server": 0,
    "n_sim_per_map": 0,
    "persistent_server_cfg_file": get_config_dir()
    + "/gibson_persistent_servers/local.yaml",
}
...
def house31(**kwargs):
    return DatasetVisualGibson(
        hd5_files=glob.glob(
            get_data_dir()
            + "/minirccar_agent_local_240fov_house31_farwp_v2/train_*.hd5"
        ),
        **kwargs
    )
...
def space8(**kwargs):
    return DatasetVisualGibson(
        hd5_files=glob.glob(
            get_data_dir() + "/minirccar_agent_local_240fov_space8_farwp_v2/train_*.hd5"
        ),
        **kwargs
    )
...
def space8_pairwise_destination(**kwargs):
    return DatasetVisualGibson(
        hd5_files=glob.glob(
            get_data_dir()
            + "/minirccar_agent_local_240fov_space8_pairwise_destination/trajs.hd5"
        ),
        **kwargs
    )
...
def make(env_name, sparsifier):
    common_kwargs["camera_pos"] = (
        sparsifier.g.get("camera_x", 0.065),
