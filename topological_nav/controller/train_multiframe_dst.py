...
# from . import args, networks, train_fixture
import args
import networks
...
if __name__ == "__main__":
    ...
    vis = visdom.Visdom(
        env=g.get("visdom_env", "main"), server=g.visdom_server, port=g.visdom_port
    )
    if not g.resume:
        vis.close()
    ...
    train_funcs[g.model_variant](
        nets={name: spec["net"] for name, spec in nets.items()},
        net_opts={name: spec["opt"] for name, spec in nets.items()},
        dataset=dataset,
        vis=vis,
        global_args=g,
    )
