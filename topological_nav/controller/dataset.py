...
class DatasetSourceTargetPair(DatasetVisualGibson):
    ...
    def _compute_overlap(self, src_sample, dst_sample, map_name):
        ...
        overlap_ratios = map.view_overlap(
            src_sample["pos"],
            src_sample["heading"],
            self.fov,
            dst_sample["pos"],
    ...
    def _get_sample(self, idx, distance):
        ...
        while True:
            ...
            while 0 <= dst_idx < len(traj):
                ...
                if (
                    d > 0.01
                    and math.cos(src_heading) * x + math.sin(src_heading) * y < 0
                ):
            ...
            if not skip and 0 <= dst_idx < len(traj):
                ...
                if (
                    overlap_ratios[0] > self.overlap_ratio
                    and overlap_ratios[1] > self.overlap_ratio
                ):
                    break
    ...
    def __getitem__(self, idx):
        ...
        heading_diff_xy = np.array(
            [math.cos(heading_diff), math.sin(heading_diff)], np.float32
        )
        ...
        try:
            ...
            print(
                "Error during making samples. dataset",
                self.hd5_files[dataset_idx],
                "traj_id",
                traj_id,
                "sample_idx",
                dst_idx,
    ...
    def __getitem__(self, idx):
        ...
        if self.future:
            for i in range(self.n_frame - 1):
                dst_samples.append(
                    traj[min(dst_idx + (i + 1) * self.frame_interval, len(traj) - 1)]
                )
        ...
        try:
            ...
            dst_imgs, dst_positions, _, _, _ = self._make(
            ...
            if distance > 0:
                if self.use_gt_wp:
                    ...
                    wp = rotate_2d(
                        dst_positions[self.n_frame - 1] - src_position, -src_heading
                    )
...
if __name__ == "__main__":
    ...
    gibson_kwargs = {
        ...
        "n_filler_server": 0,
        "n_sim_per_map": 0,
        "persistent_server_cfg_file": os.path.join(
            get_config_dir(), "gibson_persistent_servers/local.yaml"
        ),
    }
    ...
    def test_multiframe_dst():
        dataset = DatasetSourceTargetPairMultiframeDst(
            ...
            hd5_files=[
                os.path.join(
                    get_data_dir(),
                    "minirccar_agent_local_240fov_space8_farwp_v2",
                    "train_1.hd5",
                )
            ],
            **gibson_kwargs
        ...
        for (
            idx,
            (batch_src_imgs, batch_dst_imgs, batch_waypoints, batch_extras),
        ) in enumerate(loader):
            ...
            print(
                "waypoint:",
                batch_waypoints[0].data.cpu().numpy(),
                "proximity:",
                batch_extras["proximity"][0].data.cpu().numpy(),
            )
